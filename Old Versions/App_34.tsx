...
function drawSegmentsFor(routeId){
  ...
  // Draw Ticks (Lines) with correct angle bisection
  bounds.forEach((t)=>{
    const p = pickPointAlongLine(coords, t, routeId);
    if(!p) return;

    // דוגמים נקודה קצת לפני וקצת אחרי כדי לחשב כיוון
    // אנו מקטינים את הטווח ל-0.005 כדי לקבל זווית מקומית מדויקת יותר
    const pPrev = pickPointAlongLine(coords, clamp(t-0.005,0,1), routeId);
    const pNext = pickPointAlongLine(coords, clamp(t+0.005,0,1), routeId);

    if(!pPrev || !pNext) return;

    // 1. חישוב וקטור כניסה (מנורמל)
    let dx1 = p.x - pPrev.x;
    let dy1 = p.y - pPrev.y;
    ...
    // 3. חיבור הווקטורים נותן את המשיק הממוצע (חוצה הזווית)
    let tx = dx1 + dx2;
    let ty = dy1 + dy2;
    const tLen = Math.hypot(tx, ty);

    // הגנה ממקרה שהנקודות חופפות
    if (tLen < 1e-5) {
        tx = dx2; ty = dy2;
    } else {
        tx /= tLen;
    ...
    // 4. הניצב (הקו השחור) הוא (-y, x) של המשיק
    const nx = -ty;
    const ny = tx;

    const L = 12; // אורך חצי הקו (סה"כ 24 פיקסלים)
    const x1 = p.x - nx*L;
    const y1 = p.y - ny*L;
    const x2 = p.x + nx*L;
    const y2 = p.y + ny*L;
...
const PARK_OUTLINE = "rgba(0,0,0,0)";
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            if (!segLineA || segLineA.length < 2) {
                ...
                const tickLenMeters = 60; // אותו אורך שכבר יש לך (אפשר לשנות)
                ...
                {mode === "TRIPLE" && (
                    <>
                        ...
                        <div style={{ marginTop: 10 }}>
                            <div style={{ fontSize: 16, fontWeight: 900, marginBottom: 3, opacity: 0.9, padding: 12 }}>
                                שלב 6: עריכה ידנית של ישויות (הוספה/מחיקה)
                            </div>
                        ...
                        <div style={{ background: "rgba(255,255,255,0.06)", borderRadius: 12, padding: 12, marginBottom: 12 }}>
                            <div style={{ fontWeight: 900, marginBottom: 8 }}>שלב 7: הגדרת מטלה + פריסת ישויות אוטומטית</div>
                        ...
                        <div>
                            <div style={{ fontWeight: 900, marginBottom: 8 }}>שלב 8: תוצאות וויזואליזציות</div>
