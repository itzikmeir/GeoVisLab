...
function boundsFromLines(lines: LngLat[][]): maplibregl.LngLatBounds | null {
    let b: maplibregl.LngLatBounds | null = null;
    for (const line of lines) {
        for (const c of line) {
            if (!b) b = new maplibregl.LngLatBounds(c as any, c as any);
            else b.extend(c as any);
        }
    }
    return b;
}

function circleRing(center: LngLat, radiusMeters: number, steps = 56): LngLat[] {
    const pts: LngLat[] = [];
    for (let i = 0; i <= steps; i++) {
        const a = (i / steps) * Math.PI * 2;
        const dx = Math.cos(a) * radiusMeters;
        const dy = Math.sin(a) * radiusMeters;
        pts.push(offsetMeters(center, dx, dy));
    }
    return pts;
}

function sublineByDistance(line: LngLat[], startMeters: number, lengthMeters: number): LngLat[] {
    if (line.length < 2) return [];
    const cum = cumulativeDistances(line);
    const total = cum[cum.length - 1] || 1;
    const s = Math.max(0, Math.min(total, startMeters));
    const e = Math.max(s, Math.min(total, s + Math.max(1, lengthMeters)));

    const coords: LngLat[] = [];
    coords.push(pointAtDistance(line, cum, s));

    for (let i = 1; i < line.length - 1; i++) {
        if (cum[i] > s && cum[i] < e) coords.push(line[i]);
    }

    coords.push(pointAtDistance(line, cum, e));
    return coords;
}
...
function ensureTollTagImage(map: maplibregl.Map) {
    if (map.hasImage("toll-tag")) return;
    ...
    const img = ctx.getImageData(0, 0, w, h);
    map.addImage("toll-tag", { width: w, height: h, data: img.data }, { pixelRatio: 2 });
}
...
function ensureOverlay(map: maplibregl.Map) {
    ...
    // ---------- Categories overlays ----------
    // Traffic segments (red dashed over route)
    if (!map.getLayer("cat-traffic-line")) {
        map.addLayer({
            id: "cat-traffic-line",
            type: "line",
            source: "cat-traffic",
    ...
    // Toll road stripes (two yellow lines at the sides)
    ...
    // Toll tag (yellow rounded label + ₪ icon)
    if (!map.getLayer("cat-toll-tag-bg")) {
        map.addLayer({
            id: "cat-toll-tag-bg",
            type: "symbol",
            source: "cat-toll-tags",
            layout: {
                "icon-image": "toll-tag",
                "icon-size": 1,
                "icon-anchor": "center",
    ...
    if (!map.getLayer("cat-toll-tag-text")) {
        map.addLayer({
            id: "cat-toll-tag-text",
            type: "symbol",
            source: "cat-toll-tags",
            layout: {
                "text-field": ["to-string", ["get", "label"]],
    ...
    // Communication zones (purple transparent areas)
    if (!map.getLayer("cat-comm-fill")) {
        map.addLayer({
            ...
            source: "cat-comm",
            paint: {
                "fill-color": "rgba(168,85,247,0.22)",
                "fill-opacity": 1,
            },
        });
    }
    ...
    // categories above routes, below badges/segments
    moveSafe("cat-comm-fill");
    moveSafe("cat-comm-outline");
    moveSafe("cat-toll-left");
    moveSafe("cat-toll-right");
    moveSafe("cat-toll-tag-bg");
    moveSafe("cat-toll-tag-text");
    moveSafe("cat-traffic-line");
...
export default function App() {
    ...
    // categories controls (enabled after route calculation)
    const [catGlobalVariance, setCatGlobalVariance] = useState(0.35);

    const [trafficCount, setTrafficCount] = useState(3);
    const [trafficLenMeters, setTrafficLenMeters] = useState(650);
    const [trafficVariance, setTrafficVariance] = useState(0.45);

    const [tollCount, setTollCount] = useState(2);
    const [tollLenMeters, setTollLenMeters] = useState(850);
    const [tollVariance, setTollVariance] = useState(0.45);

    const [commCount, setCommCount] = useState(2);
    const [commDiameterMeters, setCommDiameterMeters] = useState(520);
    const [commVariance, setCommVariance] = useState(0.45);
    ...
    function clearCategoriesOnMap(map: maplibregl.Map) {
        setFC(map, "cat-traffic", fcLines([]));
        setFC(map, "cat-toll", fcLines([]));
        setFC(map, "cat-toll-tags", fcPoints([]));
        setFC(map, "cat-comm", fcPolygons([]));
    }

    function setCategoryLayersVisibility(map: maplibregl.Map) {
        ...
        // traffic
        setVis("cat-traffic-line", showCatTraffic);

        // toll
        setVis("cat-toll-left", showCatToll);
        setVis("cat-toll-right", showCatToll);
        setVis("cat-toll-tag-bg", showCatToll);
        setVis("cat-toll-tag-text", showCatToll);

        // comm
        setVis("cat-comm-fill", showCatComm);
        setVis("cat-comm-outline", showCatComm);
    }

    function regenerateCategories(map: maplibregl.Map) {
        const linesObj = tripleLinesRef.current;
        const linesAll: LngLat[][] = [linesObj.A, linesObj.B, linesObj.C].filter((l) => l && l.length >= 2);
        const b = boundsFromLines(linesAll);
        if (!b) {
            clearCategoriesOnMap(map);
            return;
        }
        ...
        const pickLine = (idx: number) => linesAll[Math.max(0, Math.min(linesAll.length - 1, idx % linesAll.length))];

        const effVar = (localVar: number) => clamp01(0.5 * catGlobalVariance + 0.5 * localVar);

        // deterministic-ish base fraction + jitter by variance
        const frac2 = (i: number, n: number, v: number) => {
            const fx0 = (i + 1) / (n + 1);
            const fy0 = ((i * 0.6180339887) % 1) * 0.9 + 0.05;
            const jx = (Math.random() - 0.5) * v * 0.55;
            const jy = (Math.random() - 0.5) * v * 0.55;
            const fx = Math.max(0.06, Math.min(0.94, fx0 + jx));
            const fy = Math.max(0.06, Math.min(0.94, fy0 + jy));
            return { fx, fy };
        };

        // ---- Traffic segments (on route) ----
        const trafficFeatures: { coords: LngLat[]; props?: Record<string, any> }[] = [];
        for (let i = 0; i < trafficCount; i++) {
            const line = pickLine(i + 1);
            const cum = cumulativeDistances(line);
            const total = cum[cum.length - 1] || 1;

            const v = effVar(trafficVariance);
            const len = Math.max(50, Math.min(2000, lerp(trafficLenMeters, randBetween(50, 2000), v)));

            const baseStart = ((i + 1) / (trafficCount + 1)) * Math.max(1, total - len);
            const jitter = (Math.random() - 0.5) * v * 0.65 * Math.max(1, total - len);
            const start = Math.max(0, Math.min(Math.max(0, total - len), baseStart + jitter));

            const seg = sublineByDistance(line, start, len);
            if (seg.length >= 2) trafficFeatures.push({ coords: seg, props: { kind: "traffic" } });
        }

        // ---- Toll segments (on route) + tags ----
        const tollFeatures: { coords: LngLat[]; props?: Record<string, any> }[] = [];
        const tollTags: { coord: LngLat; label: string }[] = [];
        for (let i = 0; i < tollCount; i++) {
            const line = pickLine(i + 2);
            const cum = cumulativeDistances(line);
            const total = cum[cum.length - 1] || 1;

            const v = effVar(tollVariance);
            const len = Math.max(50, Math.min(2000, lerp(tollLenMeters, randBetween(50, 2000), v)));

            const baseStart = ((i + 1) / (tollCount + 1)) * Math.max(1, total - len);
            const jitter = (Math.random() - 0.5) * v * 0.65 * Math.max(1, total - len);
            const start = Math.max(0, Math.min(Math.max(0, total - len), baseStart + jitter));

            const seg = sublineByDistance(line, start, len);
            if (seg.length >= 2) {
                tollFeatures.push({ coords: seg, props: { kind: "toll" } });

                // tag at segment midpoint
                const segCum = cumulativeDistances(seg);
                const segTotal = segCum[segCum.length - 1] || 1;
                const mid = pointAtDistance(seg, segCum, segTotal / 2);
                tollTags.push({ coord: mid, label: "₪" });
            }
        }

        // ---- Communication zones (random centers inside route-bounds) ----
        const commPolys: { coords: LngLat[]; props?: Record<string, any> }[] = [];
        for (let i = 0; i < commCount; i++) {
            const v = effVar(commVariance);
            const dia = Math.max(100, Math.min(1000, lerp(commDiameterMeters, randBetween(100, 1000), v)));
            const r = dia / 2;

            const { fx, fy } = frac2(i, Math.max(1, commCount), v);
            const lng = lerp(W, E, fx);
            const lat = lerp(S, N, fy);

            const ring = circleRing([lng, lat], r, 64);
            commPolys.push({ coords: ring, props: { kind: "comm", diameter_m: Math.round(dia) } });
        }

        // apply to map
        setFC(map, "cat-traffic", fcLines(trafficFeatures));
        setFC(map, "cat-toll", fcLines(tollFeatures));
        setFC(
            map,
            "cat-toll-tags",
            fcPoints(
                tollTags.map((t) => t.coord),
                tollTags.map((t) => ({ label: t.label }))
            )
        );
        setFC(map, "cat-comm", fcPolygons(commPolys));

        // keep visibility as current toggles
        setCategoryLayersVisibility(map);
    }
    ...
    // Apply categories visibility toggles
    useEffect(() => {
        const map = mapRef.current;
        if (!map || !map.isStyleLoaded()) return;
        setCategoryLayersVisibility(map);
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            // categories: initial random spread (within routes bounds)
            regenerateCategories(map);
            ...
            >
                ...
                <div style={{ background: "rgba(255,255,255,0.06)", borderRadius: 12, padding: 12, marginBottom: 12 }}>
                    ...
                    {[
                        ...
                        // overlays
                        { label: "עומס תנועה (קטגוריה)", v: showCatTraffic, set: setShowCatTraffic },
                        { label: "כביש אגרה (קטגוריה)", v: showCatToll, set: setShowCatToll },
                        { label: "תקשורת (קטגוריה)", v: showCatComm, set: setShowCatComm },
                    ].map((x) => (
                ...
                {mode === "TRIPLE" && (
                    <div style={{ background: "rgba(255,255,255,0.06)", borderRadius: 12, padding: 12, marginBottom: 12 }}>
                        ...
                        {/* Categories panel (available only after routes are calculated) */}
                        {badges.length > 0 && (
                            <div style={{ marginTop: 12, background: "rgba(255,255,255,0.06)", borderRadius: 12, padding: 12 }}>
                                <div style={{ fontWeight: 900, marginBottom: 6 }}>פאנל קטגוריות מרחביות</div>
                                <div style={{ fontSize: 12, opacity: 0.78, marginBottom: 12 }}>
                                    הקטגוריות מפוזרות רנדומלית <b>רק</b> באזור התוחם את שלושת המסלולים.
                                </div>

                                {/* Global variance + randomize */}
                                <div style={{ display: "flex", gap: 10, alignItems: "center", marginBottom: 10 }}>
                                    <button
                                        onClick={() => {
                                            const map = mapRef.current;
                                            if (!map) return;
                                            if (!map.getSource("cat-traffic")) return;
                                            regenerateCategories(map);
                                        }}
                                        style={{
                                            flex: 1,
                                        ...
                                        title="פיזור רנדומלי מחדש בתוך האזור התוחם את המסלולים"
                                    >
                                        פזר רנדומלי
                                    </button>

                                    <div style={{ width: 150 }}>
                                        <div style={{ fontWeight: 800, fontSize: 12, marginBottom: 6 }}>שונות כללית</div>
                                        <input
                                            type="range"
                                            min={0}
                                            max={1}
                                            step={0.01}
                                            value={catGlobalVariance}
                                            onChange={(e) => setCatGlobalVariance(parseFloat(e.target.value))}
                                            style={{ width: "100%" }}
                                        />
                                        <div style={{ fontSize: 12, opacity: 0.8, marginTop: 4 }}>{Math.round(catGlobalVariance * 100)}%</div>
                                    </div>
                                </div>

                                {/* Traffic */}
                                <div style={{ padding: "10px 0", borderTop: "1px solid rgba(255,255,255,0.08)" }}>
                                    <div style={{ fontWeight: 900, marginBottom: 8 }}>עומס תנועה</div>

                                    <div style={{ fontWeight: 800, fontSize: 12, marginBottom: 6 }}>כמות (0-8)</div>
                                    <input type="range" min={0} max={8} step={1} value={trafficCount} onChange={(e) => setTrafficCount(parseInt(e.target.value))} style={{ width: "100%" }} />
                                    <div style={{ fontSize: 12, opacity: 0.8, marginTop: 4 }}>{trafficCount}</div>

                                    <div style={{ fontWeight: 800, fontSize: 12, marginTop: 10, marginBottom: 6 }}>אורך קטע (50m–2km)</div>
                                    <input
                                        type="range"
                                        min={50}
                                        max={2000}
                                        step={50}
                                        value={trafficLenMeters}
                                        onChange={(e) => setTrafficLenMeters(parseInt(e.target.value))}
                                        style={{ width: "100%" }}
                                    />
                                    <div style={{ fontSize: 12, opacity: 0.8, marginTop: 4 }}>{trafficLenMeters} m</div>

                                    <div style={{ fontWeight: 800, fontSize: 12, marginTop: 10, marginBottom: 6 }}>שונות לוקאלית</div>
                                    <input
                                        type="range"
                                        min={0}
                                        max={1}
                                        ...
                                        onChange={(e) => setTrafficVariance(parseFloat(e.target.value))}
                                        style={{ width: "100%" }}
                                    />
                                    <div style={{ fontSize: 12, opacity: 0.8, marginTop: 4 }}>{Math.round(trafficVariance * 100)}%</div>
                                </div>

                                {/* Toll */}
                                <div style={{ padding: "10px 0", borderTop: "1px solid rgba(255,255,255,0.08)" }}>
                                    <div style={{ fontWeight: 900, marginBottom: 8 }}>כביש אגרה</div>

                                    <div style={{ fontWeight: 800, fontSize: 12, marginBottom: 6 }}>כמות (0-8)</div>
                                    <input type="range" min={0} max={8} step={1} value={tollCount} onChange={(e) => setTollCount(parseInt(e.target.value))} style={{ width: "100%" }} />
                                    <div style={{ fontSize: 12, opacity: 0.8, marginTop: 4 }}>{tollCount}</div>
                                    ...
                                    <input
                                        ...
                                        step={50}
                                        value={tollLenMeters}
                                        onChange={(e) => setTollLenMeters(parseInt(e.target.value))}
                                        style={{ width: "100%" }}
                                    />
                                    <div style={{ fontSize: 12, opacity: 0.8, marginTop: 4 }}>{tollLenMeters} m</div>
                                    ...
                                    <input
                                        ...
                                        step={0.01}
                                        value={tollVariance}
                                        onChange={(e) => setTollVariance(parseFloat(e.target.value))}
                                        style={{ width: "100%" }}
                                    />
                                    <div style={{ fontSize: 12, opacity: 0.8, marginTop: 4 }}>{Math.round(tollVariance * 100)}%</div>
                                </div>

                                {/* Communication */}
                                <div style={{ padding: "10px 0", borderTop: "1px solid rgba(255,255,255,0.08)" }}>
                                    <div style={{ fontWeight: 900, marginBottom: 8 }}>תקשורת (אזור)</div>

                                    <div style={{ fontWeight: 800, fontSize: 12, marginBottom: 6 }}>כמות (0-3)</div>
                                    <input type="range" min={0} max={3} step={1} value={commCount} onChange={(e) => setCommCount(parseInt(e.target.value))} style={{ width: "100%" }} />
                                    <div style={{ fontSize: 12, opacity: 0.8, marginTop: 4 }}>{commCount}</div>

                                    <div style={{ fontWeight: 800, fontSize: 12, marginTop: 10, marginBottom: 6 }}>קוטר (100m–1km)</div>
                                    <input
                                        type="range"
                                        min={100}
                                        max={1000}
                                        step={50}
                                        value={commDiameterMeters}
                                        onChange={(e) => setCommDiameterMeters(parseInt(e.target.value))}
                                        style={{ width: "100%" }}
                                    />
                                    <div style={{ fontSize: 12, opacity: 0.8, marginTop: 4 }}>{commDiameterMeters} m</div>
                                    ...
                                    <input
                                        ...
                                        step={0.01}
                                        value={commVariance}
                                        onChange={(e) => setCommVariance(parseFloat(e.target.value))}
                                        style={{ width: "100%" }}
                                    />
                                    <div style={{ fontSize: 12, opacity: 0.8, marginTop: 4 }}>{Math.round(commVariance * 100)}%</div>
                                </div>

                                <div style={{ fontSize: 12, opacity: 0.7, marginTop: 8 }}>
                                    טיפ: אחרי שינוי פרמטרים (כמות/אורך/קוטר/שונות) לחץ שוב על <b>פזר רנדומלי</b> כדי ליישם.
                                </div>
                            </div>
                        )}
