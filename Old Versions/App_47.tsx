...
function buildParticipantHtml(args: {
    ...
    const payload = {
        ...
        vizConfig: [
            ...
            { key: "scenicScore", label: "נוף", desc: "מעבר ליד פארקים", color: "#A855F7" },
            { key: "commScore", label: "קליטה", desc: "מעגלי תקשורת מוגברת", color: "#22C55E" },
        ],
        colors: {
            routePalette: { A: "#3B82F6", B: "#F59E0B", C: "#A855F7" },
            routeSelected: "#1E4ED8", // Dark Blue for selection
            routeRegular: "#8CCBFF",
            traffic: "#FF0022",
            toll: "#FFE100",
...
<style>
  * { box-sizing: border-box; user-select: none; }
  /* FONT UPDATE: Arial globally */
  html, body { height: 100%; margin: 0; font-family: Arial, sans-serif; background: #0b0f17; color: #e8eefc; overflow: hidden; font-size: 14px; }
  ...
  /* Legend Panel */
  .legendPanel { width: 190px; padding: 12px; display: flex; flex-direction: column; justify-content: flex-start; direction: rtl; overflow-y: auto; }
  ...
  #viz-container {
    ...
    /* FIXED COLOR: Matches the selection blue */
    ...
    position: relative;
    z-index: 5; /* Ensure it stays below the connecting tab if overlap needed */
  }
  ...
  /* Separator lines between radar charts */
  .radar-chart { position: relative; flex: 1; height: 100%; display: flex; flex-direction: column; align-items: center; justify-content: center; border-left: 1px solid rgba(255,255,255,0.08); }
  .radar-chart:first-child { border-left: none; }
  .radar-badge-corner { position: absolute; top: 10px; right: 20px; z-index: 10; display:flex; align-items:center; gap:6px; font-weight:bold; font-size:12px; color:#ddd; }

  /* Gantt */
  .ganttPanel { width: 35%; min-width: 300px; padding: 12px; direction: rtl; display: flex; flex-direction: column; border-right: 1px solid rgba(255,255,255,0.1); } 
  .ganttScroll { flex: 1; overflow-y: auto; overflow-x: hidden; padding-right: 2px; }
  
  .ganttRow { display: flex; align-items: center; height: 42px; margin-bottom: 10px; cursor: pointer; transition: 0.1s; padding-right: 6px; border-radius: 6px; border: 1px solid transparent; position: relative; }
  .ganttRow:hover { background: rgba(255,255,255,0.05); }
  
  /* TAB EFFECT: Active row connects to Viz panel on the left */
  .ganttRow.active { 
      ...
      /* Stretch left to overlap the gap/border */
      margin-left: -13px; 
      padding-left: 13px; 
      width: calc(100% + 13px);
      z-index: 10; 
...
function drawMap() {
    ...
    if(filters.routes) {
        // Z-Index Fix: Sort routes so picked is drawn last
        ...
        routeKeys.forEach(rid => {
            ...
            // Draw halo only if not selected or maybe for all? Halo for selected is usually not needed as much
...
function renderVizContainer() {
    ...
    // Fixed border color (as requested) matches the active gantt selection
    // already set in CSS, but ensured here if logic changes
    ...
    if (vizType === 'HEATMAP') {
        ...
        segments.forEach(seg => {
             ...
             // Increased Size
             const size = 200; // Increased
             const padding = 50;
             const totalSize = size + padding * 2;
             const cx = totalSize/2, cy = totalSize/2, r = size * 0.45;
             const labelR = r + 25; 
             ...
             cats.forEach((c, i) => {
                 ...
                 // Labels
                 const lblX = cx + Math.cos(ang) * labelR;
                 const lblY = cy + Math.sin(ang) * labelR;
                 
                 const gLbl = document.createElementNS(svgNS, "g");
                 
                 // Category Name (Text)
                 const nameTxt = document.createElementNS(svgNS, "text");
                 nameTxt.setAttribute("x", lblX); 
                 nameTxt.setAttribute("y", lblY - 12); // Above the score box
                 ...
                 nameTxt.textContent = c.label;

                 // Score Box
                 const rectW = 28, rectH = 16;
                 ...
                 scoreTxt.setAttribute("x", lblX); scoreTxt.setAttribute("y", lblY + 4);
                 scoreTxt.setAttribute("text-anchor", "middle");
                 scoreTxt.setAttribute("fill", "#fff");
                 scoreTxt.setAttribute("font-size", "11");
                 scoreTxt.setAttribute("font-weight", "bold");
                 scoreTxt.textContent = Math.round(seg[c.key]||0);
                 
                 gLbl.appendChild(nameTxt);
                 gLbl.appendChild(rect);
                 gLbl.appendChild(scoreTxt);
             ...
             poly.setAttribute("fill-opacity", "0.4"); // Brighter/More distinct
...
// Map Controls with Correct Zoom Logic
els.mapStage.addEventListener('wheel', e => {
    ...
    // Zoom towards pointer: calculate new pan
