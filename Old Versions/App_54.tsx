...
export default function App() {
    ...
    const generateSmartEntities = useCallback(() => {
        ...
        // 1. חישוב זמני בסיס (ללא ישויות חדשות) כדי לדעת את המצב ההתחלתי
        // אנחנו שומרים את הישויות הידניות אם המשתמש ביקש
        ...
        // חישוב זמנים נוכחי (רק עם הידניים)
        const baseScores = computeRouteScores(map, lines, manualTraffic, manualToll, manualComm, manualParksRef.current);

        // מיפוי זמנים: [RouteId, TotalTimeSeconds]
        let routeTimes = baseScores.map(s => ({ id: s.route, time: s.totalTimeS }));
        routeTimes.sort((a, b) => a.time - b.time); // מהמהיר לאיטי

        const fastest = routeTimes[0];
        const slowest = routeTimes[routeTimes.length - 1];
        const currentGapMin = (slowest.time - fastest.time) / 60;

        // רשימות חדשות להתחלה (מכילות את הידניים)
        const nextTraffic = [...manualTraffic];
        const nextToll = [...manualToll];
        const nextComm = [...manualComm];

        // 2. פרמטרים מופחתים (פחות עומס ויזואלי)
        // מקסימום 2-3 ישויות למסלול, וגם זה רק בצפיפות גבוהה
        const maxItemsTotal = Math.max(1, Math.round(scatterDensity * 4));
        const minLen = 1200; // מקטעים ארוכים יותר (1.2 ק"מ)
        const maxLen = 3500;

        // פונקציית עזר ליצירת מקטע
        const createSeg = (rid: BadgeId, type: 'traffic' | 'toll') => {
            const line = lines[rid];
            const total = polylineMeters(line);
            if (total < minLen + 500) return null;

            // מנסים למקם באמצע המסלול ולא בקצוות
            const len = randBetween(minLen, Math.min(total * 0.4, maxLen));
            const startD = randBetween(total * 0.1, total * 0.9 - len);

            return {
                coords: subLineBetweenMeters(line, startD, startD + len),
                id: `auto_${type}_${rid}_${Date.now()}_${Math.random().toString(36).substr(2, 4)}`
            };
        };

        // 3. לוגיקת איזון (Balancing)
        // המטרה: להוסיף עומס למסלול המהיר ביותר עד שהפער מצטמצם ל-targetTimeGap
        // או עד שמגיעים למכסת הצפיפות.

        let itemsCreated = 0;

        // לולאה שמנסה "להעניש" את המסלול המהיר ביותר בכל איטרציה
        // אנו רצים כל עוד הפער גדול מהרצוי ויש לנו "תקציב" ישויות
        while (
            (routeTimes[routeTimes.length - 1].time - routeTimes[0].time) / 60 > targetTimeGap &&
            itemsCreated < maxItemsTotal
        ) {
            const leaderId = routeTimes[0].id; // המסלול המהיר כרגע

            // נוסיף לו עומס (או אגרה אם המטלה היא "חסכוני")
            const typeToAdd = taskPrimaryCat === "חסכוני" ? 'toll' : 'traffic';
            const seg = createSeg(leaderId, typeToAdd);

            if (seg) {
                if (typeToAdd === 'traffic') nextTraffic.push(seg);
                else nextToll.push(seg);
                itemsCreated++;

                // הערכה גסה: כל ק"מ עומס מוסיף בערך 2 דקות (תלוי במהירות שהגדרת בחישוב)
                // כדי שהלולאה תתעדכן בלי לחשב מחדש כבד את כל המסלול, נוסיף פנדלטי מלאכותי לזמן של המסלול הזה לצורך המיון הבא
                const addedMinutes = (polylineMeters(seg.coords) / 1000) * 2.5;
                routeTimes[0].time += addedMinutes * 60;

                // מיון מחדש - אולי עכשיו מישהו אחר הוא המהיר ביותר?
                routeTimes.sort((a, b) => a.time - b.time);
            } else {
                break; // אי אפשר להוסיף עוד למסלול הזה
            }
        }

        // 4. השלמות לפי מטלה (אם עדיין לא הגענו למכסה וצריך "צבע" למפה)
        // אם המטלה "מחובר", נוסיף קליטה למסלול הארוך/איטי כדי לפצות
        if (taskPrimaryCat === "מחובר" && itemsCreated < maxItemsTotal) {
            // הוסף קליטה למסלול שאינו המוביל בזמן (כדי לתת לו יתרון אחר)
            const underdog = routeTimes[routeTimes.length - 1].id;
            const line = lines[underdog];
            const total = polylineMeters(line);
            const center = pointAtDistance(line, cumulativeDistances(line), total * 0.5);
            nextComm.push({
                id: `auto_comm_${underdog}_${Date.now()}`,
                ring: circleRing(center, 600),
                radiusM: 600
            });
        }
        ...
        // ניקוי תוצאות כדי שהמשתמש ילחץ שוב על "הצג תוצאות"
        setShowResults(false);
        setRouteScores([]);

        showToast(`בוצע איזון: נוספו ${itemsCreated} אילוצים להשוואת זמנים.`);

    }, [mapRef, tripleLinesRef, taskPrimaryCat, scatterDensity, targetTimeGap, keepManualEntities, buildTollLabelsFromSegs, showToast]);
                    ...
                    <>
                        ...
                        <div>
                            ...
                            <div style={{ background: "rgba(255,255,255,0.06)", borderRadius: 12, padding: 12, marginBottom: 20 }}>
                                ...
                                <div style={{ display: "flex", gap: 10, marginBottom: 12 }}>
                                    <button
                                        ...
                                        style={{
                                            ...
                                            borderRadius: 10,
                                            border: "1px solid rgba(255,255,255,0.15)",
                                            background: showResults ? "rgba(34, 197, 94, 0.2)" : "rgba(255,255,255,0.06)", // ירוק כפעיל
                                    ...
                                    <button
                                        ...
                                        disabled={!showResults} // פעיל רק אם יש תוצאות
                                        style={{
                                            flex: 0.4,
                                            padding: "10px",
                                            borderRadius: 10,
                                            ...
                                            background: "rgba(255,255,255,0.04)",
                                            color: "#e8eefc",
                                            cursor: showResults ? "pointer" : "not-allowed",
                                            fontWeight: 900,
                                            fontSize: 14,
                                ...
                                {/* INLINE RESULTS CONTAINER (במקום המודל הצף) */}
