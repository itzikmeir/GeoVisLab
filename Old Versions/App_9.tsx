...
function ensureBadgeImage(map: maplibregl.Map) {
    ...
    ctx.fillStyle = `rgba(${rgb.r},${rgb.g},${rgb.b},${ROUTE_OPACITY})`;
    ctx.fill();
    ctx.lineWidth = 2;
    ctx.strokeStyle = "rgba(0,0,0,0.18)";
    ctx.stroke();
...
function turnAngles(line: LngLat[]) {
    // returns array length = line.length, with angle at i (1..n-2) else 0
    const n = line.length;
    ...
    for (let i = 1; i < n - 1; i++) {
        ...
        // approximate in meters for direction stability
        ...
        // turning severity: 0 (straight) .. 180 (u-turn)
        ang[i] = deg;
    }
    return ang;
}
...
function findSplitIndices3(line: LngLat[]): { i1: number; i2: number } {
    ...
    // segment length constraints (not too short / not too long)
    const minFrac = 0.18;
    const maxFrac = 0.48;
    ...
    // pick top turning points
    const turningSorted = [...candidates]
        .filter((i) => ang[i] >= 12) // ignore tiny wiggles
        .sort((a, b) => ang[b] - ang[a])
        .slice(0, 24);

    // always include near 1/3 and 2/3
    ...
    const scorePair = (i1: number, i2: number) => {
        ...
        // objectives:
        const turnScore = (ang[i1] || 0) + (ang[i2] || 0); // bigger turns preferred
        const closeness = (1 - Math.abs(cum[i1] - d1) / total) + (1 - Math.abs(cum[i2] - d2) / total); // closer to thirds
    ...
    // fallback to thirds with small adjustment
    let i1 = near1;
    let i2 = near2;
    if (i2 <= i1) i2 = Math.min(n - 2, i1 + 1);

    // ensure minLen by nudging
    ...
    // try to satisfy min constraints roughly
...
export default function App() {
    ...
    // ✅ NEW: selected route (A default)
    const [selectedRoute, setSelectedRoute] = useState<BadgeId>("A");

    // ✅ NEW: store computed triple lines for segmentation
    ...
    // ✅ NEW: update selected route highlight + segment overlays (ONLY for selected)
    useEffect(() => {
        const map = mapRef.current;
        if (!map) return;
        if (!map.isStyleLoaded()) return;
        ...
        const line =
            selectedRoute === "A" ? lines.A : selectedRoute === "B" ? lines.B : lines.C;
        ...
        // midpoints by distance (center of each segment)
        ...
        // ticks at boundaries i1 and i2 (perpendicular, short)
        const tickLenMeters = 18; // קצר
        ...
        setFC(
            map,
            "triple-seg-ticks",
            fcLines([{ coords: tick1 }, { coords: tick2 }])
        );
        ...
        <div style={{ display: "flex", width: "100vw", height: "100vh", overflow: "hidden" }}>
            ...
            >
                ...
                <div style={{ fontSize: 13, opacity: 0.85, marginBottom: 12 }}>
                    מצב נוכחי:{" "}
                    <b>{mode === "TRIPLE" ? "3 מסלולים" : mode === "SINGLE" ? "מסלול יחיד" : "מדידה"}</b>
                </div>
                ...
                {mode === "TRIPLE" && (
                    <div style={{ background: "rgba(255,255,255,0.06)", borderRadius: 12, padding: 12, marginBottom: 12 }}>
                        ...
                        <div style={{ marginTop: 10, fontSize: 12, opacity: 0.7 }}>
                            הערה: חלוקת מקטעים (1/2/3 + טיקים) מוצגת רק למסלול שנבחר.
                        </div>
                    </div>
                )}
                ...
                <div style={{ fontSize: 12, opacity: 0.65 }}>
                    טיפ: בחר מסלול א/ב/ג בפאנל כדי להציג עליו את חלוקת המקטעים.
                </div>
            </div>
        </div>
    );
