...
function applyLabelLang(map: maplibregl.Map, lang: LabelLang) {
    ...
    for (const l of layers) {
        ...
        try {
            ...
            // טיפ קטן: כדי שתוויות RTL יהיו יותר “עקביות”
...
function ensurePinImage(map: maplibregl.Map) {
    ...
    // ✅ Convert canvas -> ImageData -> {width,height,data} (TS-friendly)
...
function ensureOverlay(map: maplibregl.Map) {
    ...
    // למצוא שכבת טקסט קיימת בסטייל ולדוגם את ה-text-font האמיתי
    ...
    try {
        ...
        console.warn("Failed to register end-pin image", e);
    }
    ...
    try { map.moveLayer("triple-labels-dot"); } catch { }
...
export default function App() {
    ...
    useEffect(() => {
        ...
        map.on("load", () => {
            ...
            console.log(
                "triple-labels features:",
                (map.getSource("triple-labels") as any)?._data
            );
            try { map.moveLayer("triple-labels"); } catch { }
            try { map.moveLayer("start-label"); } catch { }
            try { map.moveLayer("end-label"); } catch { }
    ...
    const anyRoutingLoading =
        singleState === "loading" || stateA === "loading" || stateB === "loading" || stateC === "loading";
    const anyFallback =
        singleState === "fallback" || stateA === "fallback" || stateB === "fallback" || stateC === "fallback";
