...
function buildParticipantHtml(args: {
    ...
    const payload = {
        ...
        vizConfig: [
            ...
            // כאן ההחלפה בוצעה: נוף קיבל ירוק (#22C55E)
            { key: "scenicScore", label: "נוף", desc: "מעבר ליד פארקים", color: "#22C55E" },
            // וקליטה קיבלה סגול (#A855F7)
...
<style>
  ...
  * { box-sizing: border-box; user-select: none; font-family: Arial, Helvetica, sans-serif !important; }
  ...
  .recLabel { font-size: 16px; font-weight: normal; color: #e8eefc; }
  .recVal { font-size: 16px; font-weight: 800; color: #60A5FA; }
  ...
  /* Bottom Panel Layout - RTL ORDER: 1.Gantt(Right), 2.Viz(Center), 3.Legend(Left) */
  ...
  #viz-container {
    ...
    border: 2px solid #3B82F6; /* Fixed blue border */
    background: rgba(255, 255, 255, 0.02);
    border-radius: 8px; /* Small radius on all corners */
    padding: 10px;
    display: flex;
    flex-direction: column;
    position: relative;
  ...
  /* Narrow First Column + Text align left */
  .heatmap-table td:first-child { width: 30px; padding-left: 2px; text-align: right !important; white-space: nowrap; overflow: hidden;}
  ...
  .radar-chart { 
      ...
      /* Middle Border Logic: Handled in JS based on index */
  }
  ...
  /* Active State (Matches Viz Border Color) */
  .ganttRow.active { 
...
function renderVizContainer() {
    ...
    if (vizType === 'HEATMAP') {
        ...
        segments.forEach((seg, idx) => {
             ...
             // קו הפרדה בין הרדארים (רק בין אלמנטים)
             if (idx > 0) {
             ...
             const size = 240; 
             // הגדלנו את הפאדינג כדי שיהיה מקום לפונט הגדול בצדדים
             const padding = 100; 
             const totalSize = size + padding * 2;
             const cx = totalSize/2, cy = totalSize/2, r = size * 0.7;
             const labelR = r + 35; // מרחיק את התוויות מהמעגל
             ...
             cats.forEach((c, i) => {
                 ...
                 // טקסט הציון (בתוך הקופסה)
                 const scoreTxt = document.createElementNS(svgNS, "text");
                 scoreTxt.setAttribute("x", lblX); 
                 scoreTxt.setAttribute("y", lblY + 6);
                 scoreTxt.setAttribute("text-anchor", "middle");
                 scoreTxt.setAttribute("fill", "#fff");
                 scoreTxt.setAttribute("font-size", "14"); // גודל הציון
                 scoreTxt.setAttribute("font-weight", "bold");
                 scoreTxt.textContent = Math.round(seg[c.key]||0);
                 
                 // שם הקטגוריה (ליד הקופסה)
                 const nameTxt = document.createElementNS(svgNS, "text");
                 const isLeft = Math.cos(ang) < 0;
                 
                 // אם אנחנו בצד שמאל: הטקסט יהיה משמאל לקופסה (end)
                 // אם אנחנו בצד ימין: הטקסט יהיה מימין לקופסה (start)
                 const textGap = 10; // רווח בין הקופסה לשם
                 const textX = isLeft ? (lblX - rectW/2 - textGap) : (lblX + rectW/2 + textGap);
                 const textAnchor = isLeft ? "end" : "start";
                 
                 nameTxt.setAttribute("x", textX); 
                 nameTxt.setAttribute("y", lblY + 5); // יישור אנכי עם הציון
                 nameTxt.setAttribute("text-anchor", textAnchor);
                 nameTxt.setAttribute("fill", "#e2e8f0"); // צבע בהיר כמעט לבן
                 nameTxt.setAttribute("font-size", "20"); // הגדלת פונט שם הקטגוריה
