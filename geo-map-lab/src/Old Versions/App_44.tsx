...
function buildParticipantHtml(args: {
    ...
    badges: { id: string; coord: LngLat }[]; // <--- וודא שזה קיים ומקבל את המיקום העדכני
    ...
    const payload = {
        ...
        colors: {
            ...
            routeSelected: "#1E4ED8",   // כחול כהה לנבחר
            routeRegular: "#8CCBFF",    // תכלת ללא נבחר
            traffic: "#FF0022",
            toll: "#FFE100",
...
<div class="backdrop" id="filterModal">
  <div class="modal" style="width:320px">
    ...
    <div style="margin-top:20px"><button class="mBtn no" id="closeFilter">סגור</button></div>
  </div>
</div>
...
<div class="backdrop" id="confirmModal">
  <div class="modal">
    ...
    <div id="confirmWarning" style="background:rgba(245,158,11,0.2); color:#FCD34D; padding:10px; border-radius:6px; margin-bottom:20px; display:none; font-size:14px; border:1px solid rgba(245,158,11,0.4)">שים לב: בחרת במסלול שונה מהמלצת המערכת.</div>
    <div class="modalBtns"><button class="mBtn yes" id="doConfirm">כן, אני בטוח</button><button class="mBtn no" id="cancelConfirm">ביטול</button></div>
  </div>
</div>
...
function drawMap() {
    ...
    // סדר שכבות: gOver הוא הכי עליון ולכן התגיות יהיו מעל הכל
    ...
    if(filters.routes) {
        ...
        // --- ציור תגיות המסלולים (Badges) ללא הרגליים ---
    // ... בתוך drawMap בתוך buildParticipantHtml ...

        // --- ציור תגיות המסלולים (Badges) ---
        if(DATA.badges) DATA.badges.forEach(b => {
             ...
             // --- שינויי גודל כאן ---
             const SIZE = 18;          // גודל חדש (במקום 24)
             const HALF = SIZE / 2;    // חצי גודל (למרכוז)
             const RADIUS = 4;         // רדיוס פינות (במקום 6)
             const FONT_SIZE = 13;     // גודל פונט (במקום 14)
             const TEXT_Y_OFFSET = 4;  // מיקום טקסט אנכי

             const rect = document.createElementNS('http://www.w3.org/2000/svg','rect');
             // חישוב המיקום כך שהריבוע יהיה ממורכז סביב הנקודה p
...
// ... שאר הפונקציות (renderGantt, renderViz וכו') נשארות ללא שינוי ...
