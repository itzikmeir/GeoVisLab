...
// --- Helper: Distance form point to segment squared (for speed) ---
...
// --- Helper: Min distance from point to an entire Polyline ---
// מחזיר מרחק במעלות (בערך). לצורך השוואה זה מספיק.
...
export default function App() {
    ...
    const generateSmartEntities = useCallback(() => {
        ...
        // --- 2. זיהוי המסלול הקצר ביותר (ה"פייבוריט" הטבעי) ---
        // אנחנו רוצים "להעניש" אותו במקומות שבהם האחרים מתפצלים ממנו
        const routes: BadgeId[] = ['A', 'B', 'C'];
        const lengths = routes.map(r => ({ id: r, len: polylineMeters(lines[r]), line: lines[r] }));
        lengths.sort((a, b) => a.len - b.len); // מיין מהקצר לארוך

        const shortRoute = lengths[0]; // המסלול הקצר ביותר
        const otherRoutes = lengths.slice(1); // המסלולים האחרים (הארוכים/עוקפים)

        // --- 3. לוגיקה חכמה: "מציאת הקיצור" (Split Detection) ---
        // נסרוק את המסלול הקצר. נחפש מקטעים שבהם הוא *רחוק* מהמסלולים האחרים.
        // שם בדיוק צריך לשים את הפקק/אגרה שמסבירים למה האחרים עשו עיקוף!

        if (scatterDensity > 0.1) {
            const sampleStepMeters = 50; // דגימה כל 50 מטר
            const divergenceThresholdDeg = 0.0008; // בערך 80-100 מטר מרחק כדי להחשב "פיצול"

            const totalLen = shortRoute.len;
            let currentUniqueSegmentStart = -1;

            // סוג הישות המענישה (אם המשימה "חסכוני" נשים אגרה, אחרת פקק)
            const punishmentType = taskPrimaryCat === "חסכוני" ? 'toll' : 'traffic';

            // רצים לאורך המסלול הקצר
            for (let d = 0; d < totalLen; d += sampleStepMeters) {
                // מצא את הנקודה במרחק d
                const cum = cumulativeDistances(shortRoute.line);
                const p = pointAtDistance(shortRoute.line, cum, d);

                // בדוק האם הנקודה הזו רחוקה מ*כל* המסלולים האחרים
                let isUnique = true;
                for (const other of otherRoutes) {
                    const distToOther = getMinDistToPolyline(p, other.line);
                    if (distToOther < divergenceThresholdDeg) {
                        isUnique = false; // קרוב מדי למסלול אחר (חפיפה)
                        break;
                    }
                }
                ...
                if (isUnique) {
                    ...
                    // נגמר מקטע ייחודי (חזרנו לחפיפה או לצומת)
                    if (currentUniqueSegmentStart !== -1) {
                        const segLen = d - currentUniqueSegmentStart;
                        // אם המקטע הייחודי ארוך מספיק (למשל 200 מטר), נשים בו מכשול!
                        if (segLen > 250) {
                        ...
                        currentUniqueSegmentStart = -1; // איפוס
                    }
                }
            }
        }
        ...
        // --- 4. השלמת "רעש" (Filler) ---
        // כדי שהמפה לא תהיה ריקה מדי, נוסיף עוד קצת ישויות אקראיות (אבל פחות מהרגיל)
        // כמו שעשינו קודם, אבל בזהירות לא להרוס את הלוגיקה שיצרנו

        const densityFactor = Math.max(0, Math.round(scatterDensity * 3)); // פחות אגרסיבי
        const minLen = 1000;
        const maxLen = 3000;
        ...
        const createSeg = (rid: BadgeId, type: 'traffic' | 'toll') => {
            ...
            return {
                ...
                id: `auto_filler_${type}_${rid}_${Date.now()}_${Math.random()}`
            };
        };

        // פיזור משלים (רק אם המשתמש רוצה צפיפות גבוהה)
        if (scatterDensity > 0.3) {
            for (let i = 0; i < densityFactor; i++) {
                // הוספת תקשורת (לא מפריע לניתוב, מוסיף צבע)
                ...
                // מדי פעם נוסיף עוד עומס סתמי במקום אחר
                if (Math.random() > 0.5) {
                    const rRand = routes[Math.floor(Math.random() * 3)];
                    // לא להוסיף למסלול הקצר אם כבר שמנו עליו את ה"חכמים" (כדי לא להרוג אותו לגמרי)
                    if (rRand !== shortRoute.id) {
                        const seg = createSeg(rRand, 'traffic');
        ...
        showToast(`בוצע: הוצבו חסמים בנקודות הפיצול של מסלול ${shortRoute.id} + מילוי כללי.`);
