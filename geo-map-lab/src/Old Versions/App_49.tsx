...
function renderVizContainer() {
    ...
    if (vizType === 'HEATMAP') {
        ...
        segments.forEach((seg, idx) => {
             ...
             const size = 260; 
             // 1. הגדלה משמעותית של ה-Padding כדי שהתוויות הרחוקות לא ייחתכו
             const padding = 160; 
             const totalSize = size + padding * 2;
             
             const cx = totalSize/2, cy = totalSize/2, r = size * 0.7;
             
             // 2. הרחקה נוספת מהמרכז (היה r+35, עכשיו r+65)
             const labelR = r + 65; 
             ...
             cats.forEach((c, i) => {
                 ...
                 // --- יצירת התווית המאוחדת ---
                 const lblX = cx + Math.cos(ang) * labelR;
                 const lblY = cy + Math.sin(ang) * labelR;
                 
                 const gLbl = document.createElementNS(svgNS, "g");
                 
                 // 3. הרחבת המלבן כדי שיכיל את כל הטקסט בנוחות
                 const rectW = 170; // הורחב מ-120 ל-160
                 const rectH = 40;  // הוגבה מעט ל-34
                 ...
                 rect.setAttribute("rx", 17); // פינות עגולות (חצי מהגובה)
                 rect.setAttribute("fill", c.color);
                 
                 // טקסט מאוחד
                 const textEl = document.createElementNS(svgNS, "text");
                 textEl.setAttribute("x", lblX); 
                 textEl.setAttribute("y", lblY + 6); // כיוונון אנכי למרכז
                 textEl.setAttribute("text-anchor", "middle");
                 textEl.setAttribute("fill", "#fff");
                 // פונט גדול ובולט
                 textEl.setAttribute("style", "font-size: 40px !important; font-weight: 900; font-family: Arial, sans-serif; text-shadow: 0 1px 2px rgba(0,0,0,0.3);");
