...
// בתוך buildParticipantHtml, בתוך הפונקציה drawMap...

    if(filters.toll) DATA.catTollSegs.forEach(seg => {
         // 1. הוספת הצל (חדש) - קו שחור רחב ושקוף מתחת
         // שיניתי לרוחב 10 ושקיפות 0.35 כדי לחקות את הצל במערכת
         const shadow = mkPath(seg.coords, 'rgba(0,0,0,0.35)', 10);
         gToll.appendChild(shadow);

         // 2. הקווים הצהובים המקוריים (נשארים אותו דבר)
    ...
    if(filters.routes) {
        ...
        if(DATA.badges) DATA.badges.forEach(b => {
             ...
             // Route badge: rectangle + "מסלול א" label (matches the planning app)
