...
function buildParticipantHtml(args: {
    ...
    anchors?: { id: string; coord: LngLat }[];
...
<style>
  * { box-sizing: border-box; user-select: none; }
  ...
  /* Controls (Zoom + Filter) - Shared Style */
  ...
  .ctrlBtn:hover { background: rgba(255,255,255,0.15); border-color: rgba(255,255,255,0.4); }
  ...
  /* Filter Button Specifics (uses ctrlBtn style but positioned at bottom) */
  #openFilter { position: absolute; left: 10px; bottom: 10px; z-index: 20; }
  
  /* Active State Dot for Filter */
  ...
  .gSeg { height: 100%; flex-shrink: 0; display: flex; align-items: center; justify-content: center; font-size: 13px; font-weight: 900; color: #000000; background: #8CCBFF; border-left: 3px solid #000000; box-sizing: border-box; }
...
<div class="root">
  <div class="top">
    <div class="mapPanel panel" id="mapPanel">
      <div id="mapStage">
      ...
      <button class="ctrlBtn" id="openFilter" title="שכבות">
...
function drawMap() {
    ...
    // סדר שכבות: הוספנו את gRoutesBadgeLines (רגליים) לפני gOver
    const gRoutesBadgeLines = g('grbl');

    els.svg.append(gComm, gParks, gToll, gRoutesHalo, gRoutes, gTraffic, gTollIco, gSegs, gRoutesBadgeLines, gOver);
    ...
    if(filters.routes) {
        // מיון כך שהנבחר יצויר אחרון
        const routesOrder = ['A', 'B', 'C'].filter(id => id !== picked);
        routesOrder.push(picked);

        routesOrder.forEach(rid => {
        ...
        // --- ציור תגיות המסלולים (Badges) המוקטנות ---
        if(DATA.badges) DATA.badges.forEach(b => {
             const isSel = b.id === picked;
             const p = project(b.coord);
             
             const fill = isSel ? DATA.colors.routeSelected : DATA.colors.routeRegular;
             const textColor = isSel ? '#fff' : '#000';
             ...
             rect.setAttribute('x', p.x - HALF); 
             rect.setAttribute('y', p.y - HALF); 
             ...
             const t = text(p.x, p.y + TEXT_Y_OFFSET, heb(b.id), FONT_SIZE, textColor); 
...
// -------------------------------------------------------------------
// ✅ FIX: Accurate Zoom to Pointer & Center
// -------------------------------------------------------------------

// פונקציה כללית לזום לנקודה ספציפית (עכבר או מרכז)
function zoomAtPoint(x, y, factor) {
    const newScale = Math.max(1, Math.min(10, scale * factor));
    // הנוסחה: המיקום החדש הוא המיקום הנוכחי פחות השינוי היחסי סביב נקודת העוגן
...
els.mapStage.addEventListener('wheel', e => {
    e.preventDefault();
    // חשוב: מחשבים את מיקום העכבר ביחס ל-Panel (הקונטיינר) ולא ביחס ל-Stage שזז
...
// כפתורי זום - מתמרכזים למרכז הפאנל
document.getElementById('zoomIn').onclick = () => {
    const cx = els.mapPanel.offsetWidth / 2;
    const cy = els.mapPanel.offsetHeight / 2;
...
document.getElementById('zoomOut').onclick = () => {
    const cx = els.mapPanel.offsetWidth / 2;
    const cy = els.mapPanel.offsetHeight / 2;
    zoomAtPoint(cx, cy, 1/1.2);
};

document.getElementById('fitView').onclick = () => { scale=1; pan={x:0,y:0}; updateTransform(); };

function updateTransform(){
    // שימוש במידות הקונטיינר והתוכן כדי לחשב גבולות
    const containerW = els.mapPanel.offsetWidth;
    const containerH = els.mapPanel.offsetHeight;
    const contentW = els.mapStage.offsetWidth * scale;
    const contentH = els.mapStage.offsetHeight * scale;

    // הגבלת Pan כדי לא לצאת מהתמונה
    // מקסימום Pan הוא 0 (הצמדה לשמאל/למעלה)
    // מינימום Pan הוא רוחב הקונטיינר פחות רוחב התוכן (הצמדה לימין/למטה)
    const minX = containerW - contentW;
    const minY = containerH - contentH;

    if (minX < 0) { // רק אם התוכן גדול מהקונטיינר
        if (pan.x > 0) pan.x = 0;
        if (pan.x < minX) pan.x = minX;
    } else {
        pan.x = (containerW - contentW) / 2; // מרכוז אם התמונה קטנה מהמסך
    }
