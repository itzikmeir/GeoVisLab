...
const ROUTE_COLOR = "#4AA3FF"; // כחול בהיר זהה לכל המסלולים
...
// ---------- Overlay: start/end + 3 routes + labels ----------
function ensureOverlay(map: maplibregl.Map) {
    ...
    addSrc("start-end");      // מוצא/יעד
    addSrc("triple-a");
    addSrc("triple-b");
    addSrc("triple-c");
    addSrc("triple-labels");  // א/ב/ג

    // --- load pin icon (once) ---
    const pinSvg = `
    <svg xmlns="http://www.w3.org/2000/svg" width="34" height="34" viewBox="0 0 24 24">
      <path fill="#e11d48" d="M12 22s7-4.5 7-11a7 7 0 1 0-14 0c0 6.5 7 11 7 11z"/>
    ...
    if (!map.hasImage("end-pin")) {
        map.loadImage(pinUrl, (err, img) => {
            if (!err && img && !map.hasImage("end-pin")) {
                map.addImage("end-pin", img);
            }
        });
    }
    ...
    // --- start/end labels: "מוצא" / "יעד" ---
    if (!map.getLayer("start-end-labels")) {
        map.addLayer({
            id: "start-end-labels",
            type: "symbol",
            source: "start-end",
    ...
    // triple routes — כולם אותו צבע כחול בהיר
    const addTripleLine = (id: string) => {
    ...
    addTripleLine("triple-a");
    addTripleLine("triple-b");
    addTripleLine("triple-c");
...
export default function App() {
    ...
    // render start/end points source (מוצא/יעד)
    useEffect(() => {
        const map = mapRef.current;
        if (!map) return;
